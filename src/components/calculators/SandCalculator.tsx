...
export default function SandCalculator() {
  ...
  const debouncedPrice = useDebouncedValue(pricePerYard);
  ...
  const results: CalcResultItem[] = useMemo(() => {
    ...
    const density = 1.35; // tons per cubic yard for sand
    ...
    if (price > 0) {
      ...
      res.push({ label: 'Estimated Cost', value: '$' + cost.toFixed(2), unit: '' });
    }

    return res;
  }, [debouncedLength, debouncedWidth, debouncedDepth, depthUnit, debouncedPrice]);
  ...
  return (
    <div className="border-2 border-primary-200 dark:border-[#525252] rounded-xl p-5 sm:p-6 bg-white dark:bg-[#2a2a2a]">
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <button
          type="button"
          onClick={() => { setLength(''); setWidth(''); setDepth(''); setPricePerYard(''); }}
