...
export default function RiverRockCalculator() {
  ...
  const results: CalcResultItem[] = useMemo(() => {
    ...
    if (price > 0) {
      ...
      resultItems.push({ label: 'Estimated Cost', value: '$' + cost.toFixed(2), unit: '' });
    }

    return resultItems;
  }, [debouncedLength, debouncedWidth, debouncedDepth, depthUnit, rockSize, debouncedPrice]);
  ...
  return (
    <div className="border-2 border-primary-200 dark:border-[#525252] rounded-xl p-5 sm:p-6 bg-white dark:bg-[#2a2a2a]">
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <button
          type="button"
          onClick={() => { setLength(''); setWidth(''); setDepth(''); setRockSize('1-3'); setPricePerTon(''); }}
