...
export default function SidingCalculator() {
  ...
  const results: CalcResultItem[] = useMemo(() => {
    ...
    return [
      { label: 'Total Wall Area', value: totalArea.toFixed(2), unit: 'sq ft' },
      { label: 'Net Area (after openings)', value: netArea.toFixed(2), unit: 'sq ft' },
      { label: 'Squares (100 sq ft units)', value: squares.toFixed(2), unit: 'squares' },
      { label: 'Recommended Total (w/ waste)', value: totalWithWaste.toFixed(2), unit: 'sq ft' },
    ];
  }, [debouncedWallHeight, debouncedWallWidth, debouncedNumWalls, debouncedOpeningsArea, debouncedWasteFactor]);
  ...
  return (
    <div className="border-2 border-primary-200 dark:border-[#525252] rounded-xl p-5 sm:p-6 bg-white dark:bg-[#2a2a2a]">
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <button
          type="button"
          onClick={() => { setWallHeight(''); setWallWidth(''); setNumWalls(''); setOpeningsArea(''); setWasteFactor(''); }}
